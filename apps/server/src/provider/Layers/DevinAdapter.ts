...
function encodeDevinResume(sessionId: string): {
  schemaVersion: typeof DEVIN_RESUME_VERSION;
  sessionId: string;
} {
  return { schemaVersion: DEVIN_RESUME_VERSION, sessionId };
}
    ...
    const startSession: DevinAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const requestedStartModelId = startModelSelection?.model
            ? resolveDevinAcpBaseModelId(startModelSelection.model)
            : undefined;
          ...
          const displayModel = boundModelId ? resolveDevinAcpBaseModelId(boundModelId) : undefined;

          const boundModeId = yield* applyDevinAcpModeSelection({
            runtime: acp,
          ...
          const session: ProviderSession = {
            ...
            threadId: input.threadId,
            resumeCursor: encodeDevinResume(started.sessionId),
            createdAt: now,
            updatedAt: now,
          };
              ...
              Effect.gen(function* () {
                ...
                switch (event._tag) {
                  ...
                  case "ConfigOptionsUpdated":
                    ctx.currentModelId = currentDevinModelIdFromConfigOptions(event.configOptions);
                    return;
                  case "AssistantItemStarted":
                    if (!canEmitTurnEvent) return;
                    yield* offerRuntimeEvent(
                      makeAcpAssistantItemEvent({
                        stamp,
                        provider: PROVIDER,
                    ...
                    yield* offerRuntimeEvent(
                      makeAcpThreadTokenUsageUpdatedEvent({
                        ...
                        ...(typeof event.outputTokens === "number"
                          ...
                          : {}),
                        ...(typeof event.cachedReadTokens === "number"
        ...
        const prepared = yield* withThreadLock(
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const currentModeId = yield* applyDevinAcpModeSelection({
                runtime: ctx.acp,
