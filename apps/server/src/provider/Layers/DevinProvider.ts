...
const runDevinAuthStatusCommand = (
  devinSettings: DevinSettings,
  environment: NodeJS.ProcessEnv = process.env,
) =>
  Effect.gen(function* () {
...
function devinAuthFromStatusOutput(output: {
  readonly code: number;
  readonly stdout: string;
  readonly stderr: string;
}): { status: "ready" | "error"; message?: string } {
  const combined = `${output.stdout}\n${output.stderr}`;
  if (output.code === 0 && /Logged in/.test(combined)) {
    return { status: "ready" };
  }
  return {
    status: "error",
    message: "Devin CLI is not authenticated. Run `devin auth login`.",
  };
}
...
> {
  ...
  const authResult = yield* runDevinAuthStatusCommand(devinSettings, environment).pipe(
    Effect.timeoutOption(AUTH_PROBE_TIMEOUT_MS),
    Effect.result,
  );

  if (Result.isFailure(authResult)) {
    yield* Effect.logWarning("Devin auth status check failed.", {
    ...
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "error",
        auth: { status: "unknown" },
        message: "Devin CLI is installed but `devin auth status` failed.",
      },
    });
  }
  ...
  if (Option.isNone(authResult.success)) {
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "error",
        auth: { status: "unknown" },
        message: "Devin CLI is installed but `devin auth status` timed out.",
      },
    });
  }

  const authStatus = devinAuthFromStatusOutput(authResult.success.value);
  ...
  if (Exit.isFailure(discoveryExit)) {
    ...
    return buildServerProvider({
      ...
      probe: {
        ...
        message:
          "Devin CLI is authenticated but ACP startup failed. Check server logs for details.",
      },
    });
  }
  if (Option.isNone(discoveryExit.value)) {
    ...
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "error",
        auth: { status: "authenticated" },
        message: `Devin CLI is authenticated but ACP startup timed out after ${DEVIN_ACP_MODEL_DISCOVERY_TIMEOUT_MS}ms.`,
      },
    });
  }
